...
class Portfolio(models.Model):
    ...
    def __str__(self):
        return f"Portfolio de {self.user.username} - {self.balance} $"
...
class Position(models.Model):
    ...
    def __str__(self):
        return f"{self.symbol} ({self.quantity}) - {self.portfolio.user.username}"
...
class Prediction(models.Model):
    ...
    profit_loss     = models.DecimalField(max_digits=10, decimal_places=2,
                                          null=True, blank=True,
                                          verbose_name="Profit / Perte potentiel")
    created_at      = models.DateTimeField(auto_now_add=True,
