...
class MetricsView(APIView):
    ...
    def get(self, request):
        data = MetricsService.get_metrics(request.user)
...
_SYMBOLS = ["AAPL", "NVDA", "BTC-USD", "ETH-USD", "MSFT", "GOOGL", "TSLA", "AMZN"]
_MOCK_FALLBACK = [
...
class MarketPricesView(APIView):
    ...
    def get(self, request):
        ...
        with ThreadPoolExecutor(max_workers=8) as executor:
            futures = {executor.submit(fetch_one, s): s for s in _SYMBOLS}
        ...
        for symbol in _SYMBOLS:
            ...
            if data.get("valid", True) and data.get("price", 0) > 0:
                change = data.get("change_pct", 0.0) or (
                    abs(data.get("price", 0) - 100) / 100
                    * (1 if data.get("price", 0) > 100 else -1)
                )
                prices.append({
                    "symbol": symbol,
        ...
        if not prices:
            logger.warning("MarketPricesView: all ML calls failed — returning mock fallback")
            return Response(_MOCK_FALLBACK)
